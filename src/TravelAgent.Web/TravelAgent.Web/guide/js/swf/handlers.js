...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
    ...
    $.ajax({
                ...
                success: function(result){
                     if(result.status ==0){
                         obj.customSettings.queue = obj.customSettings.queue || new Array();
                         while (obj.customSettings.queue.length > 0) {              
                              obj.cancelUpload(obj.customSettings.queue.pop(), false);
                         }
                         ajax_login();                          
			 return false;
                     }else{
...
function uploadStart(file) {
	...
	try {
		 ...
		 */
                ...
                if(this.customSettings.queue.length>this.settings.file_queue_limit)
                {
                    while(this.customSettings.queue.length>this.settings.file_queue_limit)
                    {
