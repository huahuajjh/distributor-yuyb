...
{
    public partial class VisaOrderList : TravelAgent.Web.UI.BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
                ...
                {
                    DataBindState();
                    RptBind("a.orderType=" + Convert.ToInt32(TravelAgent.Tool.EnumSummary.OrderType.签证) + this.CombSqlTxt(this.strordercode, this.strordername, this.strorderstartdate, this.strorderenddate, this.orderstate, this.clubid), "a.orderDate desc");

                    DateTime dtStartDate;
                    DateTime dtEndDate;
                    this.hljrorder.NavigateUrl = "/admin/visa/VisaOrderList.aspx?osd=" + DateTime.Now.ToString("yyyy-MM-dd") + "&oed=" + DateTime.Now.ToString("yyyy-MM-dd");
                    TravelAgent.Tool.DateHelper.GetDateOfWeek(DateTime.Now, out dtStartDate, out dtEndDate);
                    this.hlbzorder.NavigateUrl = "/admin/visa/VisaOrderList.aspx?osd=" + dtStartDate.ToString("yyyy-MM-dd") + "&oed=" + dtEndDate.ToString("yyyy-MM-dd");
                    ...
                    this.hlbyorder.NavigateUrl = "/admin/visa/VisaOrderList.aspx?osd=" + dtStartDate.ToString("yyyy-MM-dd") + "&oed=" + dtEndDate.ToString("yyyy-MM-dd");

                    if (Admin.Role.roleAuth.IndexOf(",visaorder_delete,") <= -1)
                    {
        ...
        public string ShowEdit(string id)
        {
            ...
            if (Admin.Role.roleAuth.IndexOf(",visaorder_opr,") > -1)
            {
                sbEdit.Append("<a href=\"EditVisaOrder.aspx?id=" + id + "\" class=\"tablelink club_art\" title=\"操作订单\" width=\"730px\" height=\"550px\">操作</a>  ");
            }
            if (Admin.Role.roleAuth.IndexOf(",visaorder_delete,") > -1)
            {
        ...
        private void RptBind(string strWhere, string orderby)
        {
            ...
            //获得总条数
            this.pcount = VisaOrderBll.GetVisaOrderCount(strWhere);
            if (this.pcount > 0)
            {
            ...
            DataSet ds = VisaOrderBll.GetPageList3(this.pagesize, this.page, strWhere, orderby);
        ...
        protected string CombSqlTxt(string _ordercode, string _ordername, string _orderstartdate, string _orderenddate, string _orderstate, int _clubid)
        {
            ...
            {
                strTemp.Append(" and b.visaName like '%" + _ordername + "%'");
            }
        ...
        protected void lbtnDel_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    VisaOrderBll.Delete(id);
                }
            }
            JscriptPrint("批量删除成功！", "VisaOrderList.aspx?" + CombUrlTxt(this.strordercode, this.strordername, this.strorderstartdate, this.strorderenddate, this.orderstate, this.clubid) + "page=0", "Success");
        }
        /// <summary>
        /// 查询
        /// </summary>
        ...
        {
            Response.Redirect("VisaOrderList.aspx?" + this.CombUrlTxt(this.txtOrderCode.Text.Trim(), this.txtOrderName.Text.Trim(), this.txtOrderStartDate.Text, this.txtOrderEndDate.Text,this.ddlState.SelectedValue, this.clubid) + "page=0");
        }
        /// <summary>
        /// 绑定状态属性
        /// </summary>
        ...
        protected void lbtnExport_Click(object sender, EventArgs e)
        {
            ...
            DataSet dsclub = VisaOrderBll.GetList2(0, "orderType=" + Convert.ToInt32(TravelAgent.Tool.EnumSummary.OrderType.签证) + this.CombSqlTxt(this.strordercode, this.strordername, this.strorderstartdate, this.strorderenddate, this.orderstate, this.clubid), "orderDate desc");
        ...
        public DataTable GetJHDataTable(DataSet dsQueryClub)
        {
            ...
            dt.Columns.Add(new DataColumn("签证名称", typeof(string)));
