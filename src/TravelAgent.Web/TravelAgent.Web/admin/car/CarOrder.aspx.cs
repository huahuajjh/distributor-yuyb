...
{
    public partial class CarOrder : TravelAgent.Web.UI.BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                DataBindState();
                RptBind("a.orderType=" + Convert.ToInt32(TravelAgent.Tool.EnumSummary.OrderType.租车) + this.CombSqlTxt(this.strordercode, this.strordername, this.strorderstartdate, this.strorderenddate, this.orderstate, this.clubid), "a.orderDate desc");
                ...
                this.hlbzorder.NavigateUrl = "/admin/product/CarOrder.aspx?osd=" + dtStartDate.ToString("yyyy-MM-dd") + "&oed=" + dtEndDate.ToString("yyyy-MM-dd");
                ...
                this.hlbyorder.NavigateUrl = "/admin/product/CarOrder.aspx?osd=" + dtStartDate.ToString("yyyy-MM-dd") + "&oed=" + dtEndDate.ToString("yyyy-MM-dd");
                ...
                if (Admin.Role.roleAuth.IndexOf(",carorder_delete,") <= -1)
                {
        ...
        public string ShowEdit(string id)
        {
            ...
            if (Admin.Role.roleAuth.IndexOf(",carorder_opr,") > -1)
            {
                sbEdit.Append("<a href=\"EditCarOrder.aspx?id=" + id + "\" class=\"tablelink club_art\" title=\"操作订单\" width=\"730px\" height=\"550px\">操作</a>  ");
            }
            if (Admin.Role.roleAuth.IndexOf(",carorder_delete,") > -1)
            {
        ...
        private void RptBind(string strWhere, string orderby)
        {
            ...
            //获得总条数
            this.pcount = OrderBll.GetCarOrderCount(strWhere);
            if (this.pcount > 0)
            {
            ...
            DataSet ds = OrderBll.GetPageList4(this.pagesize, this.page, strWhere, orderby);
        ...
        /// <returns></returns>
        protected string CombSqlTxt(string _ordercode, string _ordername, string _orderstartdate, string _orderenddate,string _orderstate, int _clubid)
        {
            ...
            {
                strTemp.Append(" and b.CarName like '%" + _ordername + "%'");
            }
        ...
        /// <param name="_keywords"></param>
        /// <returns></returns>
        protected string CombUrlTxt(string _ordercode, string _ordername, string _orderstartdate, string _orderenddate, string _orderstate, int _clubid)
        {
            StringBuilder strTemp = new StringBuilder();
            if (!_ordercode.Equals(""))
            {
        ...
        {
            Response.Redirect("CarOrder.aspx?" + this.CombUrlTxt(this.txtOrderCode.Text.Trim(), this.txtOrderName.Text.Trim(), this.txtOrderStartDate.Text, this.txtOrderEndDate.Text, this.ddlState.SelectedValue, this.clubid) + "page=0");
        }
        /// <summary>
        /// 批量删除
        /// </summary>
        ...
        protected void lbtnDel_Click(object sender, EventArgs e)
        {
            ...
            JscriptPrint("批量删除成功！", "CarOrder.aspx?" + CombUrlTxt(this.strordercode, this.strordername, this.strorderstartdate, this.strorderenddate, this.orderstate, this.clubid) + "page=0", "Success");
        }
        /// <summary>
        /// 绑定状态属性
        /// </summary>
        ...
        /// <returns></returns>
        public int ShowTotal(int orderprice, int attachprice)
        {
            return orderprice + attachprice;
        }
        /// <summary>
        /// 显示租车的明细
        /// </summary>
        ...
        protected void lbtnExport_Click(object sender, EventArgs e)
        {
            ...
            DataSet dsclub = OrderBll.GetList2(0, "orderType=" + Convert.ToInt32(TravelAgent.Tool.EnumSummary.OrderType.租车) + this.CombSqlTxt(this.strordercode, this.strordername, this.strorderstartdate, this.strorderenddate, this.orderstate, this.clubid), "orderDate desc");
        ...
        public DataTable GetJHDataTable(DataSet dsQueryClub)
        {
            ...
            dt.Columns.Add(new DataColumn("用车日期", typeof(string)));
            dt.Columns.Add(new DataColumn("还车日期", typeof(string)));
            dt.Columns.Add(new DataColumn("用车数量", typeof(int)));
            dt.Columns.Add(new DataColumn("租车详细", typeof(string)));
            ...
            for (int i = 0; i <= dsQueryClub.Tables[0].Rows.Count - 1; i++)
            {
                ...
                row["订单号"] = qrow[2];
                row["下单时间"] = qrow[6];
                row["用车日期"] = qrow[27]+" "+qrow[28];
                row["还车日期"] = qrow[29];
                row["用车数量"] = qrow[30];
                row["租车详细"] = qrow[31]+" "+ShowCarPrice(Convert.ToInt32(qrow[3]));
